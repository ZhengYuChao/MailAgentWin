...
MailAgent Windows 在后台以异步事件循环和多线程任务池的方式运行，整体架构如下：
...
graph TD
    ...
    %% Subgraph CoreEngine
    subgraph CoreEngine ["【核心引擎 & 任务调度】"]
        Radar["COM Radar 监听器 com_radar.py"]:::module
        API_Server["API Webhook 服务器 server.py"]:::module
        TunnelManager["Tunnel 隧道管理器 tunnel.py"]:::module
        TaskPool["全局优先级任务池 task_pool.py"]:::module
        Watcher["WindowsWatcher new_watcher_win.py"]:::module
        SyncStore["SQLite 同步数据库 sync_store.db"]:::module
    end

    subgraph DataProcessors ["【业务处理模块】"]
        OutlookArm["Outlook COM 驱动 outlook_com_arm.py"]:::module
        DraftHandler["草稿和发信处理器 draft_handler.py"]:::module
        AttachHandler["附件与 Office 转换 office_converter_win.py"]:::module
        ConvIndex["会话索引解析器 conversation_index.py"]:::module
        NotionSync["Notion 同步驱动 sync.py"]:::module
        AIController["Notion AI 控制器 controller.py"]:::module
        FeishuNotifier["飞书通知器 feishu.py"]:::module
    end

    %% Subgraph Outputs
    subgraph Outputs ["【输出结果】"]
        NotionPage["Notion 数据库页面 树形线程关系及 Office 预览"]:::output
        OutlookDraft["Outlook 邮件回复 保存草稿或直接发送"]:::output
        FeishuCard["飞书富文本消息卡片 带 AI 优先级与摘要"]:::output
        NotionAIChat["Notion AI 侧边栏对话总结"]:::output
    end

    %% Connections
    OutlookEvents -->|实时捕获 中优先级| Radar
    OutlookHistory -->|启动扫描 低优先级| Watcher
    NotionWebhook -->|公网请求| API_Server
    DailyTimer -->|定时触发 中优先级| TaskPool

    Radar -->|添加同步任务| TaskPool
    API_Server -->|添加草稿任务 高优先级| TaskPool
    TaskPool -->|出队并分发任务| Watcher
    
    Watcher -->|协调执行| OutlookArm
    Watcher -->|协调执行| DraftHandler
    Watcher -->|协调执行| AttachHandler
    Watcher -->|协调执行| ConvIndex
    Watcher -->|协调执行| NotionSync
    Watcher -->|协调执行| AIController
    Watcher -->|协调执行| FeishuNotifier

    OutlookArm -->|读取邮件及附件数据| Watcher
    DraftHandler -->|COM 执行草稿及发信| OutlookDraft
    AttachHandler -->|Office 转换及上传| Watcher
    ConvIndex -->|查找父页面关系| SyncStore
    NotionSync -->|写入数据及存储记录| SyncStore

    NotionSync -->|创建页面| NotionPage
    FeishuNotifier -->|发送富文本卡片| FeishuCard
    AIController -->|Playwright 模拟提问| NotionAIChat
```
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **中央服务入口** | [main.py](main.py) | 全局生命周期管理，加载隧道与 API 服务，并启动 Watcher 主任务循环。 | `.env` 配置文件 | 1. 按需启动穿透隧道并启动 API 服务器线程<br>2. 启动 Notion AI 后台防抖和每日定时调度<br>3. 启动 `WindowsWatcher` 核心事件循环 | 启动日志，协调运行各服务 |
...
*   `main.py`: 服务的统一入口，调度 Outlook 监听、防抖服务、定时任务和 Webhook API 服务器。
